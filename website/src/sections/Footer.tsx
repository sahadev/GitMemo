  ...
  return (
    <footer className="py-16 px-6 border-t border-border">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-14 grid gap-6 border-y border-border/50 py-8 md:grid-cols-[minmax(0,1fr)_auto] md:items-center">
          <div className="min-w-0 text-center md:text-left">
            <p className="mb-3 inline-flex items-center justify-center gap-2 text-sm font-semibold uppercase tracking-wider text-accent md:justify-start">
              <MessageCircle size={16} />
              {t('footer.community.label')}
            </p>
            <h3 className="mb-3 text-xl font-bold text-text sm:text-2xl">{t('footer.community.title')}</h3>
            <p className="mx-auto max-w-2xl text-sm leading-7 text-text-secondary md:mx-0">
              {t('footer.community.subtitle')}
            </p>
          </div>
          <div className="mx-auto flex w-full max-w-[190px] flex-col items-center gap-3 rounded-lg border border-border bg-white p-3 shadow-[0_18px_44px_rgba(0,0,0,0.16)] md:mx-0">
            <img
              src="/gitmemo-group.png"
              alt={t('footer.community.qrAlt')}
              className="w-full rounded-md"
              loading="lazy"
            />
            <span className="text-center text-xs font-medium text-[#201d1d]">
              {t('footer.community.caption')}
            </span>
          </div>
        </div>

param(
    ...
    [string]$MacPrivateKeyPath = "lan-control/.state/ssh/lan_control_windows_ed25519",
...
$firewallRuleName = "LAN-Control-OpenSSH-Server-TCP-$Port"
...
if (-not $existingRule) {
    New-NetFirewallRule `
        ...
        -DisplayName "LAN Control OpenSSH Server TCP $Port" `
        -Enabled True `
        -Direction Inbound `
        -Protocol TCP `
        -Action Allow `
...
if ($PairingServerUrl) {
    try {
        ...
        Write-Host ""
        Write-Host "Paired with LAN Control."
    } catch {
        Write-Host ""
